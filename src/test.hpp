...
public:
  ...
  constexpr static auto ProcessEntity(Handle_t<EntSig_t> e, Callback_t cb, ECSManager_t& ecs_man) -> void
  {
  ...
  template<class EntSig_t> constexpr static auto TraverseEntities(auto policy, auto cb, auto& ecs_man) -> void
  {
    ...
    std::for_each(
      policy, std::make_reverse_iterator(indexes.end()), std::make_reverse_iterator(indexes.begin()), [&](auto i) {
  ...
  constexpr auto CreateComponents(TList_t<Cmps_t...>, auto... cmps) -> auto
  {
