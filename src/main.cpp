...
constexpr auto rendereable_printer [[maybe_unused]] = [](const auto& ren, const auto& pos, auto ent_id) {
    std::cout << "Found entity ID:     " << ent_id.GetID() << std::endl;
...
int main()
{
    ...
    Args::Arguments_t ren_args { Args::For_v<RenderComponent_t>, 'c' };
    Args::Arguments_t pos_args { Args::For_v<PositionComponent_t>, 1, 2 };
    Args::Arguments_t phy_args { Args::For_v<PhysicsComponent_t>, 3, 4 };

    auto ent1 = ecs_man.CreateEntity<Renderable_t>(ren_args);
    auto ent2 = ecs_man.CreateEntity<Renderable_t>(ren_args, pos_args);
    auto ent3 = ecs_man.CreateEntity<Movable_t>(pos_args, phy_args);
    auto ent4 = ecs_man.CreateEntity<BasicCharacter_t>(Args::Arguments_t{ Args::For_v<RenderComponent_t>, 'a' });
