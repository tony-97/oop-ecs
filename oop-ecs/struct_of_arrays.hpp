...
template<template<class...> class Container_t, class... Ts> struct SoA_t : Container_t<Ts>...
{
public:
  ...
  template<class T, class U> constexpr auto operator[](U u) const -> const auto&
  {
    return GetRequiredContainer<T>()[u];
  }
  ...
  template<class T> constexpr auto reserve(size_type<T> new_size) -> void
  {
    GetRequiredContainer<T>().reserve(new_size);
  }
  ...
  template<class T, class... Args_t> constexpr auto emplace(const_iterator<T> pos, Args_t&&... args) -> iterator<T>
  {
  ...
  template<class T> constexpr auto erase(const_iterator<T> pos) -> iterator<T>
  {
    return GetRequiredContainer<T>().erase(pos);
  }

  template<class T> constexpr auto erase(const_iterator<T> first, const_iterator<T> last) -> iterator<T>
  {
    return GetRequiredContainer<T>().erase(first, last);
  }
  ...
  template<class T> constexpr auto resize(size_type<T> count, const value_type<T>& value) -> void
  {
    GetRequiredContainer<T>().resize(count, value);
  }

  template<class T> constexpr auto static get_container_index() -> size_type<T> { return TMPL::IndexOf_v<T, Ts...>; }

  template<class T> [[nodiscard]] constexpr auto begin() const -> const_iterator<T>
  {
    return GetRequiredContainer<T>().begin();
  }

  template<class T> [[nodiscard]] constexpr auto cbegin() const -> const_iterator<T>
  {
    return GetRequiredContainer<T>().cbegin();
  }

  template<class T> [[nodiscard]] constexpr auto begin() -> iterator<T> { return GetRequiredContainer<T>().begin(); }

  template<class T> [[nodiscard]] constexpr auto rbegin() const -> const_reverse_iterator<T>
  {
    return GetRequiredContainer<T>().rbegin();
  }

  template<class T> [[nodiscard]] constexpr auto crbegin() const -> const_reverse_iterator<T>
  {
    return GetRequiredContainer<T>().crbegin();
  }

  template<class T> [[nodiscard]] constexpr auto rbegin() -> reverse_iterator<T>
  {
    return GetRequiredContainer<T>().rbegin();
  }

  template<class T> [[nodiscard]] constexpr auto end() const -> const_iterator<T>
  {
    return GetRequiredContainer<T>().end();
  }

  template<class T> [[nodiscard]] constexpr auto cend() const -> const_iterator<T>
  {
    return GetRequiredContainer<T>().cend();
  }

  template<class T> [[nodiscard]] constexpr auto end() -> iterator<T> { return GetRequiredContainer<T>().end(); }

  template<class T> [[nodiscard]] constexpr auto rend() const -> const_reverse_iterator<T>
  {
    return GetRequiredContainer<T>().rend();
  }

  template<class T> [[nodiscard]] constexpr auto crend() const -> const_reverse_iterator<T>
  {
    return GetRequiredContainer<T>().crend();
  }

  template<class T> [[nodiscard]] constexpr auto rend() -> reverse_iterator<T>
  {
    return GetRequiredContainer<T>().rend();
  }

protected:
  template<class RequiredType_t>
  [[nodiscard]] constexpr auto GetRequiredContainer() const -> const Container_t<RequiredType_t>&
  {
    CheckIfTypeExists<RequiredType_t>();
    return static_cast<const Container_t<RequiredType_t>&>(*this);
  }

  template<class RequiredType_t> [[nodiscard]] constexpr auto GetRequiredContainer() -> Container_t<RequiredType_t>&
  {
  ...
  template<class Type_t> constexpr static auto CheckIfTypeExists() -> void
  {
