...
constexpr auto movable_printer [[maybe_unused]] = [](auto& phy, auto& pos, auto ent) {
  std::cout << "[Found entity ID    ]: " << ent.GetIndex() << std::endl;
  std::cout << "[PhysicsComponent_t ]: "
            << "VX: " << phy.vx << " VY: " << phy.vy << std::endl;
  std::cout << "[PositionComponent_t]: "
            << "X: " << pos.x << " Y: " << pos.y << std::endl
            << std::endl;
};
...
main() -> int
{
  ...
  ecs_man.ForEach<BasicCharacter_t>([&](auto&, auto&, auto&, auto&& ent) { ecs_man.TransformTo<Movable_t>(ent); });
